...
🔗 **[now-mcp](https://github.com/jyagunii-bot/now-mcp)** — A small MCP server for Claude that does the same thing on the server side. Use it on mobile or non-Chrome environments where this extension can't run. They complement each other: the extension covers Chrome, the MCP covers everywhere else. 
...
🔗 **[now-mcp](https://github.com/jyagunii-bot/now-mcp)** — Claude用の姉妹MCPサーバー。Chrome以外の環境（モバイルなど）でも時刻を渡せるようにしたもの。両方併用すれば、どの環境でも時刻が伝わります。
